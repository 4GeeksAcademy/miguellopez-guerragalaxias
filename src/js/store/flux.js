const getState = ({ getStore, getActions, setStore }) => {
    return {
        ...
        actions: {
            ...
            cargarVehiculos: () => {
                fetch("https://www.swapi.tech/api/vehicles") // Corrección: URL sin la barra diagonal al final
                    .then(res => res.json())
            ...
            cargarFavoritos: (nombre) => {
                setStore({favoritos: getStore().favoritos.concat(nombre)}) 
            },

            eliminarFavoritos: (nombre) => {
                setStore({favoritos: getStore().favoritos.filter((element)=>element!==nombre)}) 
            },

            comprobarFavoritos: (nombre) =>  {
                const favoritos = getStore().favoritos
                if (favoritos.indexOf(nombre) !== -1) {
                    getActions().eliminarFavoritos(nombre)
                } 
                getActions().cargarFavoritos(nombre)
            } 
        }
    };
};
