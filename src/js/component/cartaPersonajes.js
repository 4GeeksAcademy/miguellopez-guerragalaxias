    ...
    return (
      <div id="carouselExample" className="carousel slide" data-bs-ride="carousel">
        ...
        <div className="carousel-inner">
          {store.personajes.map((personaje, index) => (
            <div className={`carousel-item ${index === 0 ? 'active' : ''}`} key={personaje.uid}>
              <div className="card-container">
                <div className="card" style={{ width: "18rem" }}>
                  <div className="img">
                    <img src={store.imagenesPersonajes[index]} className="card-img-top" alt={`Imagen de ${personaje.name}`} />
                  </div>
                  <div className="card-body">
                    <h5 className="card-title">{personaje.name}</h5>
                    <button className="btn btn-primary botonCard" onClick={() => {navigate(`/vista-personaje-individual/${personaje.uid}/${index}`)}}>VER MAS</button>
                    <p onClick={() => actions.comprobarFavoritos(personaje.name)}>
